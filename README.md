...
**Disease X** is the WHO's official placeholder name for an unknown pathogen that could cause a future global epidemic.
...
This project simulates the command-and-control system that a government health authority or hospital network would use during such an outbreak. It predicts patient risk in real time, auto-allocates critical resources, and tracks hospital capacity automatically.
...
|---------|--------------------------|
| Do not know which patients will deteriorate | ML model scores every patient 0-100% on admission |
| Ventilators and critical resources run out | DB trigger auto-deducts resources the moment a Critical patient is admitted |
| Cannot track capacity across multiple facilities | DB trigger updates bed occupancy instantly on every admission |
...
| Resources | Medical equipment inventory per facility |
| Pathogen_Profiles | Known and hypothetical pathogen definitions |

---

## Database Triggers

Three triggers fire **automatically** on every patient admission.

### trigger_patient_occupancy
Fires AFTER INSERT OR DELETE on Patients.
- INSERT causes current_occupancy + 1
- DELETE causes current_occupancy - 1
...
Empty result means all resources are safe. Any row means action is required.
...
├── .gitignore                          # Excludes .env and .pkl
...
Never upload your credentials or model file. Create a `.gitignore` file with:
